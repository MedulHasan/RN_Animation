...
const RegistrationRecaptchaCheckbox = ({ reCaptchaError }) => {
    const {trans} = useLangTranslation();
    const {reCaptchaToken} = useSelector(signupSelector);
    ...
    const onVerify = (token) => {
        if (Boolean(token)) {
            dispatch(storeCaptchaToken(token));
...
export default RegistrationRecaptchaCheckbox;
